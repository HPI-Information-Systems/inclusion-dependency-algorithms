...
public class Exporter {
	...
	public static void export() throws SQLException, IOException {
		...
		try {
			for (Attribute attribute : attributes) {
				...
				resultSet = getValues(attribute);
				exportAttValues(attribute, resultSet);

			}

			long et = System.currentTimeMillis();
			Performance performance = CommonObjects.getPerformnce();
			performance.addExportTime(et - st);
			System.out.println("the values of " + attributes.size() + " columns are exported.");
	...
	private static ResultSet getValues(Attribute attribute)
			throws AlgorithmExecutionException {

		final TableInputGenerator generator = attribute.getTable().getTableInputGenerator();
		return generator.sortBy(attribute.getName(), false);
	}
