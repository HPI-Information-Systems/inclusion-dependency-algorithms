...
public class ReINDeer extends ReINDeerAlgorithm implements InclusionDependencyAlgorithm, // Output
  ...
  public ArrayList<ConfigurationRequirement<?>> getConfigurationRequirements() {
    ConfigurationRequirementFileInput fileInput = new ConfigurationRequirementFileInput(
        ReINDeer.Identifier.INPUT_TABLES.name(),
    ...
    return new ArrayList<>(
        Arrays.asList(fileInput, tableInput, tempFolder, cleanTemp, detectNary, maxNaryLevel,
    ...
    if (ReINDeer.Identifier.TEMP_FOLDER_PATH.name().equals(identifier)) {
      if (this.isValidPath(values[0])) {
        throw new AlgorithmConfigurationException(
            ReINDeer.Identifier.TEMP_FOLDER_PATH + " must not be \"" + values[0] + "\"");
      }
  ...
  private void handleUnknownConfiguration(String identifier, Object[] values)
      throws AlgorithmConfigurationException {
    throw new AlgorithmConfigurationException(
        "Unknown configuration: " + identifier + " -> " + this.concat(values, ","));
  }
  ...
  private boolean isValidPath(String path) {
    // Path must be valid
    if (path == null || path.equals("") || path.equals(" ") || path.equals("/") || path.equals("\\")
    ...
    if (file.getAbsolutePath().equals(rootSlash.getAbsolutePath()) || file.getAbsolutePath()
        .equals(rootBackslash.getAbsolutePath())) {
      return false;
    }
